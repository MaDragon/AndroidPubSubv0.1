...
3. Create Cognito. In Cognito Console dashboard,
   a. Select “N.Virginia” as region.
   b. Click “Manage Federated Identities”
   c. Click “Create new identity pool”
   d. Enter a unique name to identity pool.
   e. Click “Enable access to unauthenticated identities” checkbox.
   f. Click “Create pool” to the next page, the region in the above bar becomes “Global”. And policy documents for each role shown as below, not the same as tutorial.
   g. Click “Allow” and Obtain the PoolID constant
...
4. Set up two roles in IAM (IAM does not require region selection)
   a. Click two roles in IAM
   b. Enter the role and attach policy
   c. Select “AWSIOTFullAccess” and attach policy
   d. Attach the same policy to second role
